...
public class ChatBot extends Thread
{
		...
		while (runner)
		{
			try
			{
				...
				else
				{
					...
					String answer = db.readDBData(ticketToSend.getLastText().toLowerCase());
					
					ticketToSend.addMessageToChat(EChatSender.SERVER, answer);
	...
	public void receivedMessage(String message, int ticketID)
	{
		for(int i = 0; i < tickets.size(); i++)
		{
			...
			{
				tickets.get(i).addMessageToChat(EChatSender.CLIENT, message);
